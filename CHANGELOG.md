...
- `skills-remove` now cleans up Hermes symlinks. Restore always hints to re-run
  `skills-mgr sync` to recreate Hermes links.
...
- `categorize_skill()` now returns `(display_category, hermes_dir)` instead of a
  ...
  backward-compatible.
- `skills-mgr update --apply` uses git fetch + rsync instead of `npx skills add`.
